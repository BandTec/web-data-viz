    ...
    usuarioModel.cadastrar(nome, email, senha, fkProdutor[0].produtor_id)
      .then(
        async function (resultado) {
          await codigoModel.desativarCodigo(fkProdutor[0].id)
          res.json(resultado);
        }
      ).catch(
        function (erro) {
...
async function cadastrarPerson(req, res) {
  var nome = req.body.nomeServer
  ...
  if (nome == undefined) {
    ...
    const resCodigo = await codigoModel.buscarProdutorPorCodigo(codigo)
    console.log(resCodigo)
    if (!resCodigo || resCodigo.length === 0 || resCodigo[0].produtor_id !== 1)
      return res.status(401).send("Código não autorizado");
    // Passe os valores como parâmetro e vá para o arquivo usuarioModel.js
